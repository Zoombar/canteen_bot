...
Python 3.10+, **aiogram 3**, **SQLite3**, **APScheduler**. Меню из `.docx` (почта IMAP или ручная загрузка), рассылка меню в будни в заданное время (по умолчанию 8:30, `Asia/Omsk`), заказ до дедлайна, сводка для столовой (Excel/CSV/текст), месячный отчёт (2 колонки: сотрудник, сумма).
